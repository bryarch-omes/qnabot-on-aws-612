...
function getV2CXoneTemplate(request, response) {
    const messages = [
        {
            contentType: response.type,
            content: response.message,
    ...
    if (_.get(response, 'session.customPayload')) {
        messages.push({
            contentType: 'CustomPayload',
            content: _.get(response, 'session.customPayload', 'Empty')
        });
    }
...
function assembleLexV2Response(request, response) {
    ...
    if (isConnectClientChat(request) && isInteractiveMessage(response)) {
        ...
        out = getV2CXoneTemplate(request, response);
    }
